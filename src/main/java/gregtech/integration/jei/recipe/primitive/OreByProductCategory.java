...
public class OreByProductCategory extends PrimitiveRecipeCategory<OreByProduct, OreByProduct>{
	...
	public OreByProductCategory(IGuiHelper guiHelper) {
        ...
        String baseLocation = GTValues.MODID + ":textures/gui/arrows/";
        this.arrowsBase = guiHelper.drawableBuilder(new ResourceLocation(baseLocation + "oreby-base.png"), 0, 0, 176, 166)
                .setTextureSize(176, 166).build();
        this.arrowsDirectSmelt = guiHelper.drawableBuilder(new ResourceLocation(baseLocation + "oreby-smelt.png"), 0, 0, 176, 166)
                .setTextureSize(176, 166).build();
        this.arrowsChemBath = guiHelper.drawableBuilder(new ResourceLocation(baseLocation + "oreby-chem.png"), 0, 0, 176, 166)
                .setTextureSize(176, 166).build();
        this.arrowsSeparator = guiHelper.drawableBuilder(new ResourceLocation(baseLocation + "oreby-sep.png"), 0, 0, 176, 166)
                .setTextureSize(176, 166).build();
	...
	@Override
	public void setRecipe(@NotNull IRecipeLayout recipeLayout, @NotNull OreByProduct recipeWrapper, @NotNull IIngredients ingredients) {
        ...
        for (int i = 0; i < ITEM_OUTPUT_LOCATIONS.size(); i += 2) {
            itemStackGroup.init(i / 2 + itemInputs.size(), false, new ItemStackTextRenderer(recipeWrapper.getChance(i / 2 + itemInputs.size())), ITEM_OUTPUT_LOCATIONS.get(i) + 1, ITEM_OUTPUT_LOCATIONS.get(i + 1) + 1, 16, 16, 0, 0);
        ...
        for (int i = 0; i < FLUID_LOCATIONS.size(); i += 2) {
            fluidStackGroup.init(i / 2, true, new FluidStackTextRenderer(1, false, 16, 16, null), FLUID_LOCATIONS.get(i) + 1, FLUID_LOCATIONS.get(i + 1) + 1, 16, 16, 0, 0);
	...
	public void drawExtras(Minecraft minecraft) {
        ...
        if (hasDirectSmelt) this.arrowsDirectSmelt.draw(minecraft, 0, 0);
        if (hasChemBath) this.arrowsChemBath.draw(minecraft, 0, 0);
        if (hasSeparator) this.arrowsSeparator.draw(minecraft, 0, 0);
        if (hasSifter) this.arrowsSifter.draw(minecraft, 0, 0);

        this.slot.draw(minecraft, ITEM_INPUT_LOCATIONS.get(0), ITEM_INPUT_LOCATIONS.get(1));

        for (int i = 0; i < ITEM_OUTPUT_LOCATIONS.size(); i += 2) {
            if (itemOutputExists.get(i / 2) || (i >= 28 * 2 && hasSifter)) {
                this.slot.draw(minecraft, ITEM_OUTPUT_LOCATIONS.get(i), ITEM_OUTPUT_LOCATIONS.get(i + 1));
            }
        }

        for (int i = 0; i < FLUID_LOCATIONS.size(); i += 2) {
            if (fluidInputExists.get(i / 2)) {
                this.fluidSlot.draw(minecraft, FLUID_LOCATIONS.get(i), FLUID_LOCATIONS.get(i + 1));
            }
        }
	}
